...
func TestTestConnection(t *testing.T) {
	t.Run("successful connection returns latency", func(t *testing.T) {
		...
		latency, err := client.TestConnection(&types.Connection{Host: mr.Host(), Port: port, DB: 0, UseCluster: false})
		if err != nil {
	...
	t.Run("wrong port returns error", func(t *testing.T) {
		...
		_, err := client.TestConnection(&types.Connection{Host: "127.0.0.1", Port: 1, DB: 0, UseCluster: false})
		if err == nil {
	...
	t.Run("wrong host returns error", func(t *testing.T) {
		...
		if err := c.Connect(&types.Connection{Host: mr.Host(), Port: port, DB: 0, UseCluster: false}); err != nil {
			t.Fatalf("Connect() error = %v", err)
		}
		t.Cleanup(func() { _ = c.Disconnect() })

		_, err = c.TestConnection(&types.Connection{Host: "192.0.2.1", Port: 6379, DB: 0, UseCluster: false})
		if err == nil {
